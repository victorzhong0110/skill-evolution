"""Intel-analyzer template pack for scripts/bridge_auto_respond.py.

Historical example: canned responses used while evolving the `intel-analyzer`
skill (a bilingual AI-news scoring pipeline). Preserved verbatim — topics,
entities (TeraWulf, EngineAI, SpaceX, ...), and scoring formulas are specific
to that application and are NOT part of the generic framework.
...
PACK_NAME = "intel-analyzer"

# Ordered (topic, keywords) pairs — first match wins (case-insensitive).
TOPIC_PATTERNS = (
    ("dedup", ("dedup", "duplicate", "TeraWulf")),
    ("credibility", ("single_source", "credibility", "EngineAI")),
    ("opinion_vs_event", ("opinion", "editorial", "event_nature")),
    ("scoring_formula", ("dimension", "granularity", "formula", "简单相加")),
    ("noise_floor", ("floor", "threshold", "auto-excluded", "noise")),
    ("source_independence", ("independence", "coordinated", "political control", "media")),
    ("priority_events", ("missing", "priority", "must-capture", "SpaceX")),
)
...
# Formatted with {strategy_name} and {topic} by the generic responder.
